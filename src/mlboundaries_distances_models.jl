...
function pp_df_for(epair, n_cand, method)
    file = "$EXP_DIR/$(epair.id)_bcs_$(method)_$(n_cand)_pp.csv"
    if !isfile(file)
        file = "$EXP_DIR/$(epair.id)_bcs_$(method)_$(n_cand).csv"
    end
...
function boxplots_distances(epairs, dir = EXP_DIR, n_cand = 20)
    local pl
...
function latex_table_distances(epairs, io=stdout, dir=EXP_DIR)
    latex_table_head(io)
    for epair in epairs
        for n_cand in [10, 20]
            ...
            println(io, "& $(n_cand) & $(round(d_rand_nn_mean, digits=2)) \$\\pm\$ $(round(d_rand_nn_sd, digits=2)) & \\textbf{$(round(d_div_nn_mean, digits=2))} \$\\pm\$ \\textbf{$(round(d_div_nn_sd, digits=2))} \\\\")
        end
        println(io, "\\hline")
    end
...
open("$RES_DIR/distance_table.tex", "w") do io
    latex_table_distances(epairs, io)
 end

using StatsPlots
pl = boxplots_distances(epairs)
Plots.svg(pl, "$RES_DIR/distance_violin")