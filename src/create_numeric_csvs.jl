...
df_a = CSV.read("$DATA_DIR/adult.csv", DataFrame)

df_a.gender = map(g -> g == "Male" ? 0.0 : 1.0, df_a.gender)

CSV.write("$DATA_DIR/adult_pp.csv", df_a)

# ----- titanic

df_t = CSV.read("$DATA_DIR/titanic.csv", DataFrame)
...
CSV.write("$DATA_DIR/titanic_pp.csv", df_t)

# ----- student data

df_s = CSV.read("$DATA_DIR/student_data.csv", DataFrame)
...
CSV.write("$DATA_DIR/student_data_pp.csv", df_s)

# ----- car evaluation

df_c = CSV.read("$DATA_DIR/car_evaluation.csv", DataFrame)
...
CSV.write("$DATA_DIR/car_evaluation_pp.csv", df_c)
